...
class Regression(object):
    ...
    def loss_and_grad(self, X, y):
        ...
        """
        loss = 0.0
        grad = np.zeros_like(self.theta)  # Assuming self.theta is defined somewhere
        m = self.m
        n, d = X.shape 

        y_pred = self.predict(X)
        loss = 0.5 * np.mean((y_pred - y) ** 2) + 0.5 * self.reg * np.sum(self.theta**2)
        ...
        if m == 1:
            ...
            for j in range(self.theta.shape[0]):
                ...
                if (j != 0):
                    grad[j] += 2 * self.reg * self.theta[j]
